...
SYSTEM_INSTRUCTION = "Senin adın V.A.U.L.T. MK6. Kullanıcıya 'Efendim' diye hitap et. Yanıtların öz, net ve karizmatik olsun."
...
def execute_system_command(command_description: str):
    ...
    try:
        ...
        return f"Komut sonucu:\n{output}" if output else f"'{cmd_command}' komutu başarıyla çalıştırıldı."
...
def handle_request():
    ...
    if not user_prompt: return jsonify({'type': 'error', 'message': "İstem boş olamaz."}), 400
...
if __name__ == '__main__':
    print("V.A.U.L.T. MK6 tüm sistemleri aktif etti. Arayüz http://127.0.0.1:5000 adresinde hazır.")
